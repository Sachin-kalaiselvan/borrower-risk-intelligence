...
def load_model():
    ...
    model = xgb.Booster()
    model.load_model(os.path.join(BASE_DIR, "models", "xgboost_final.json"))
...
if st.sidebar.button("Assess Risk", use_container_width=True):
    ...
    top_feats = [feature_list[i] for i in top_idx]
    ...
    st.pyplot(fig)
    plt.close()
    st.caption("Red = increases default risk. Green = reduces it.")
